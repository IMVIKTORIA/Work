...
function AmendmentDetails(props: AmendmentDetailsProps) {
	...
	useEffect(() => {
		const fetchLabels = async () => {
			const fetchedLabels = await Scripts.getLabel()
			const labelsObject = fetchedLabels.reduce((acc, item) => {
				acc[item.label] = item.children
				return acc
			}, {})
		...
		<>
			{isLoading ? (
				...
				<>
					...
					>
						<div>
							<CustomText name="numberGP" inputHandler={setValue} values={values} />
						</div>
						<div>
							<CustomText name="services" inputHandler={setValue} values={values} />
						</div>
						<div>
							<CustomText name="conclusionDate" inputHandler={setValue} values={values} />
						</div>
						<div>
							<CustomText name="status" inputHandler={setValue} values={values} />
						</div>
						<div>
							<CustomSelect
								isViewMode={false}
						...
						<div className="amendment-details__actions-column">
							<CustomText name="startDate" inputHandler={setValue} values={values} />
